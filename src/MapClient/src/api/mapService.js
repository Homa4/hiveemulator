...
const API_BASE_URL =
  "https://sea-lion-app-jjunw.ondigitalocean.app/hiveemulator-src-communication-c/api/v1/client";
...
export const fetchInterferences = async (apiUrl) => {
  try {
    ...
    const invalid = interferences.filter(
      (i) =>
        i.lat === null ||
        i.lon === null ||
        isNaN(i.lat) ||
    ...
    if (invalid.length > 0) {
      console.warn(
        `${invalid.length} interferences have invalid data and will be ignored:`,
        invalid,
      );
    }

    const valid = interferences.filter(
      (i) =>
        i.lat !== null &&
        i.lon !== null &&
        !isNaN(i.lat) &&
...
export const addInterference = async (apiUrl, lat, lon, radiusMeters) => {
  try {
    ...
    if (isNaN(latitude) || isNaN(longitude) || isNaN(radius)) {
      throw new Error(
        "Invalid input: lat, lon, and radius must be valid numbers",
      );
    }
    ...
    console.log(
      `Interference added at: ${lat}, ${lon} with radius ${radiusKM}km (${radius}m)`,
    );
    return response.data;
  } catch (error) {
